...
impl RealGitRepository {
    ...
    fn simple_git_command(&self, args: Vec<String>) -> BoxFuture<'_, Result<()>> {
        ...
        self.executor
            .spawn(async move {
                let git = git_binary?;
                let arg_refs: Vec<_> = args.iter().map(|arg| arg.as_str()).collect();
    ...
    fn drop_commit_support_impl(&self, sha: String) -> BoxFuture<'_, Result<DropCommitSupport>> {
        ...
        self.executor
            .spawn(async move {
                ...
                let mut ancestry_command =
                    git.build_command(&["merge-base", "--is-ancestor", &sha, "HEAD"]);
                let ancestry_output = ancestry_command.output().await?;
    ...
    fn drop_commit(&self, sha: String) -> BoxFuture<'_, Result<()>> {
        ...
        self.executor
            .spawn(async move {
                ...
                git.run(&["rebase", "--onto", parent_sha, &sha, "HEAD"]).await?;
                Ok(())
            })
            .boxed()
    }
